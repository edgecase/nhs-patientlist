...
describe PatientListsController do
  ...
  describe '#index' do
    ...
    it "gets the current user's custom patient lists" do
      ...
      controller.send(:patient_lists).should eq(PatientList.find_all_by_user_id(current_user.id))
    end
  end
  describe '#create' do
    let(:outpatients) { "Outpatients" }
    it "creates a new custom patient list and redirects to the index" do
      post :create, :patient_list => { "name" => outpatients}, :user_id => current_user.id
